...
SELECTED_DATE = pd.to_datetime(
    "2025-05-02T00:00:00", format='%Y-%m-%dT%H:%M:%S')
...
# File paths
VEHICLE_NUM_MAPPING_PATH = os.getenv(
    'VEHICLE_NUM_MAPPING_PATH', 'data/fleet_device_mapping.csv')
WAYBILL_METABASE_PATH = os.getenv(
    'WAYBILL_METABASE_PATH', 'data/waybill_metabase.csv')
ROUTE_STOP_MAPPING_PATH = os.getenv(
    'ROUTE_STOP_MAPPING_PATH', 'data/route_stop_mapping.csv')
GPS_DATA_PATH = os.getenv('GPS_DATA_PATH', 'data/amnex_direct_data.csv')
CACHE_OUTPUT_PATH = os.getenv(
    'CACHE_OUTPUT_PATH', f'travel_time_cache_{timestamp}.json')
ROUTE_POLYLINE_PATH = os.getenv(
    'ROUTE_POLYLINE_PATH', 'data/pgrider_route.csv')
INTEGRATED_BPP_CONFIG_PATH = os.getenv(
    'INTEGRATED_BPP_CONFIG_PATH', 'data/pgrider_integrated_bpp_config.csv')
MERCHANT_ID_PATH = os.getenv('MERCHANT_ID_PATH', 'data/pgrider_merchant.csv')
MERCHANT_OPERATING_CITY_ID_PATH = os.getenv(
    'MERCHANT_OPERATING_CITY_ID_PATH', 'data/pgrider_merchant_operating_city.csv')

# Constants
STOP_VISIT_RADIUS = float(
    os.getenv('STOP_VISIT_RADIUS', '0.05'))  # 50 meters in km
CACHE_TTL = int(os.getenv('CACHE_TTL', '3600'))  # Cache TTL in seconds
# Bus location expiry in seconds
BUS_LOCATION_MAX_AGE = int(os.getenv('BUS_LOCATION_MAX_AGE', '120'))
...
def load_csv_data():
    ...
    try:
        ...
        merchant_operating_city_id_df = pd.read_csv(
            MERCHANT_OPERATING_CITY_ID_PATH)

        merchant_id = merchant_id_df[merchant_id_df['Short ID']
                                     == 'NAMMA_YATRI']['ID'].values[0]
        ...
        filtered_route_polyline_df = route_polyline_df[
            route_polyline_df['Integrated Bpp Config ID'] == integrated_bpp_config]

        gps_df = pd.read_csv(GPS_DATA_PATH)
        logger.info(f"Before filtering dates: {len(gps_df)}")
        gps_df['Date'] = pd.to_datetime(
            gps_df['Date'], format='%Y-%m-%dT%H:%M:%S', errors='coerce')
        gps_df['Timestamp'] = pd.to_datetime(
            gps_df['Timestamp'], format='%Y-%m-%dT%H:%M:%S', errors='coerce')
        gps_df['ServerTime'] = pd.to_datetime(
            gps_df['ServerTime'], format='%Y-%m-%dT%H:%M:%S', errors='coerce')
        ...
        logger.info(
            f"Filtered GPS data for device {SELECTED_DEVICE_ID} on {SELECTED_DATE} with {len(filtered_gps_df)} points.")
...
def build_mappings():
    ...
    device_to_vehicle = dict(
        zip(device_id.astype(str), vehicle_mapping_df['Fleet']))
    ...
    vehicle_to_route = dict(
        zip(waybill_df['Vehicle No'], waybill_df['route_no']))
...
class StopTracker:
    ...
    def get_route_stops(self, route_id):
        ...
        # Get from CSV
        try:
            route_stops = route_stop_df[route_stop_df['TUMMOC Route ID'] == int(
                route_id)]
    ...
    def reset_visited_stops(self, route_id, vehicle_id, vehicle_no):
        ...
        try:
            ...
            logger.info(
                f"Reset visited stops for vehicle {vehicle_id} on route {route_id}")
            return True
        except Exception as e:
    ...
    def check_if_at_stop(self, stop, vehicle_lat, vehicle_lon):
        ...
        lat2, lon2 = math.radians(float(stop['stop_lat'])), math.radians(
            float(stop['stop_lon']))
    ...
    def find_next_stop(self, stops, visited_stops, vehicle_lat, vehicle_lon):
        ...
        if not visited_stops:
            ...
            for stop in stops:
                _, distance = self.check_if_at_stop(
                    stop, vehicle_lat, vehicle_lon)
    ...
    def get_travel_duration(self, origin_id, dest_id, origin_lat, origin_lon, dest_lat, dest_lon, device_id=None):
        ...
        try:
            ...
            if origin_id != 0:
                ...
                mean_travel_time = travel_time_cache.calculate_mean_travel_time(
                    cache_key)
    ...
    def check_if_crossed_stop(self, prev_location, current_location, stop_location, threshold_meters=20):
        ...
        is_on_path = abs(dist_prev_to_stop + dist_stop_to_curr -
                         path_distance) < threshold_meters
        ...
        # Get bearings
        bearing_prev_to_curr = calculate_bearing(
            prev_location, current_location)
        bearing_prev_to_stop = calculate_bearing(prev_location, stop_location)
        bearing_stop_to_curr = calculate_bearing(
        ...
        alignment_prev_to_stop = angle_diff(
            bearing_prev_to_curr, bearing_prev_to_stop) < 60
        alignment_stop_to_curr = angle_diff(
            bearing_prev_to_curr, bearing_stop_to_curr) < 60
    ...
    def calculate_eta(self, stopsInfo, route_id, vehicle_lat, vehicle_lon, current_time, vehicle_id, visited_stops=[], vehicle_no=None):
        ...
        if vehicle_no is not None and vehicle_id is not None:
            store_vehicle_location_history(
                vehicle_no, vehicle_lat, vehicle_lon, int(current_time.timestamp()))
            store_vehicle_location_history(
                vehicle_id, vehicle_lat, vehicle_lon, int(current_time.timestamp()))

        # Check if the vehicle is at a stop now
        for stop in stops:
            # Check if vehicle is at the stop based on current position
            is_at_stop, _ = self.check_if_at_stop(
                stop, vehicle_lat, vehicle_lon)
                        ...
                        if len(visited_stops) > 0:
                                        ...
                                        if prev_stop['stop_id'] == prev_stop_id:
                                            ...
                                            if at_prev_stop:
                                                prev_time = point.get(
                                                    'timestamp', 0)
                                                logger.info(
                                                    f"Found previous stop time for {prev_stop_id}: {prev_time}")
                                                break
                            ...
                            if prev_time is not None:
                                ...
                                if 0 < travel_time < 3600:
                                    ...
                                    travel_time_cache.set(
                                        cache_key, vehicle_id, current_time_unix, travel_time)
                                    logger.info(
                                        f"Vehicle {vehicle_id} travel time from {prev_stop_id} to {current_stop_id}: {travel_time} seconds")
                                else:
                                    logger.warning(
                                        f"Unreasonable travel time detected: {travel_time} seconds from {prev_stop_id} to {current_stop_id}")

                            logger.info(
                                f"Vehicle {vehicle_id} crossed stop {stop['stop_id']} between updates")
            ...
            if is_at_stop:
                ...
                if stop['stop_id'] not in visited_stops:
                    ...
                    logger.info(
                        f"Vehicle {vehicle_id} is at stop {stop['stop_id']}")
                    visited_stops = self.update_visited_stops(
                        route_id, vehicle_id, stop['stop_id'])
                    calculation_method = "visited_stops"
                break

        # Find next stop based on visited stops
        (next_stop, distance) = self.find_next_stop(
            stops, visited_stops, vehicle_lat, vehicle_lon)
        if next_stop:
            if not distance:
                _, distance = self.check_if_at_stop(
                    next_stop, vehicle_lat, vehicle_lon)
            ...
            closest_stop, distance = self.find_closest_stop(
                stops, vehicle_lat, vehicle_lon)
        ...
        for i in range(closest_index + 1, len(stops)):
            ...
            if duration:
                cumulative_time += duration
                arrival_time = current_time + \
                    timedelta(seconds=cumulative_time)
...
def store_vehicle_location_history(device_id: str, lat: float, lon: float, timestamp: int, max_points: int = 25):
    ...
    try:
        ...
        logger.error(
            f"Error storing vehicle history for {device_id}: {e}\nHistory value: {history}\nTraceback: {error_details}")
...
def calculate_route_match_score(route_id, vehicle_id, route_stops, location_history, max_distance_meter=100):
    ...
    try:
        ...
        if not polyline_points and isinstance(route_stops, dict) and 'stops' in route_stops:
            ...
            polyline_points = [(float(stop['stop_lat']), float(
                stop['stop_lon'])) for stop in stops]
            # Require more points when using stops
            min_points_required = min(10, len(stops) // 2)

        # Check if we have enough location history
        if not location_history or len(location_history) < min_points_required:
            logger.info(
                f"Not enough location history for route {route_id}, vehicle {vehicle_id}: {len(location_history)} points (need {min_points_required})")
            return 0.0

        # Check if we have enough polyline points
        if not polyline_points or len(polyline_points) < 2:
            logger.info(
                f"Not enough polyline points for route {route_id}: {len(polyline_points) if polyline_points else 0}")
            return 0.0

        # Sort location_history by timestamp to ensure chronological order
        location_history = sorted(
            location_history, key=lambda x: x.get('timestamp', 0))
        ...
        proximity_ratio = len(near_points) / \
            len(location_history) if location_history else 0
        ...
        if proximity_ratio >= 0.3 and len(min_segments) >= 2:
            ...
            if min(min_segments) == min_segments[0]:
                logger.info(
                    f"Route match for {route_id}, vehicle {vehicle_id}: {len(near_points)}/{len(location_history)} points, score: {proximity_ratio:.2f}")
                return proximity_ratio

        logger.info(
            f"Route match for {route_id}, vehicle {vehicle_id}: {len(near_points)}/{len(location_history)} points, rejected due to direction check")
        return 0.0
...
def get_fleet_info(device_id: str, current_lat: float = None, current_lon: float = None, timestamp: int = None) -> dict:
    ...
    # Cache validity in seconds (5 minutes) - shorter to allow route reassessment
    cache_ttl = 300
    ...
    if fleet_info is not None and current_lat is not None and current_lon is not None and timestamp is not None:
        ...
        store_vehicle_location_history(
            fleet_info['vehicle_no'], current_lat, current_lon, timestamp)
        store_vehicle_location_history(
            device_id, current_lat, current_lon, timestamp)

        # Check if we should reevaluate the route (every 300 seconds / 5 minutes or after 10 new points)
        vehicle_history = get_vehicle_location_history(
            fleet_info['vehicle_no'])
        if len(vehicle_history) > 20:  # Have enough history to make a good decision
            ...
            if current_time - last_assessment > 300:
                ...
                logger.info(
                    f"Time-based route reassessment for vehicle {fleet_info['vehicle_no']}")
            else:
                ...
                if 'route_id' in fleet_info and fleet_info['route_id']:
                    ...
                    route_stops = stop_tracker.get_route_stops(
                        current_route_id)
                    ...
                    if route_stops and 'polyline' in route_stops and route_stops['polyline']:
                        ...
                        if polyline_points:
                            ...
                            if not is_near:
                                logger.info(
                                    f"Vehicle {fleet_info['vehicle_no']} position is far from route {current_route_id} (distance: {distance*1000:.1f}m), forcing reassessment")
                                force_reassessment = True
        ...
        logger.info(
            f"Clearing cache for vehicle {device_id} to force route reassessment")
    ...
    try:
        ...
        if len(tummoc_route_ids) == 1:
            ...
            if current_lat is not None and current_lon is not None:
                store_vehicle_location_history(
                    vehicle_no, current_lat, current_lon, timestamp)
                store_vehicle_location_history(
                    device_id, current_lat, current_lon, timestamp)
            ...
            if force_reassessment and len(location_history) > 10:
                ...
                logger.info(
                    f"Using {len(recent_location_history)} recent points for route reassessment")
            else:
            ...
            if len(recent_location_history) < 5:
                logger.info(
                    f"Not enough location history for {vehicle_no}, using first route")
                route_id = list(tummoc_route_ids)[
                    0] if tummoc_route_ids else None
            else:
                ...
                if best_score >= 0.3:
                    ...
                    logger.info(
                        f"Selected best route {route_id} with score {best_score:.2f}")
                else:
                    # Fall back to first route if no good match
                    route_id = list(tummoc_route_ids)[
                        0] if tummoc_route_ids else None
                    logger.info(
                        f"No good route match, using first route {route_id}")
                    ...
                    if force_reassessment and fleet_info and 'route_id' in fleet_info:
                        ...
                        current_route_score = next(
                            (score for route, score in route_scores if route == current_route), 0.0)
                        ...
                        if current_route_score > 0.15 and best_score < 0.4:
                            ...
                            logger.info(
                                f"Keeping current route {route_id} (score: {current_route_score:.2f})")
...
def handle_gps_data():
    ...
    for device_id, group in gps_df.groupby('DeviceId'):
        points = group.sort_values('Timestamp')  # Sort by timestamp
        for i, row in points.iterrows():
            try:
                ...
                store_vehicle_location_history(
                    device_id, current_lat, current_lon, int(timestamp))
                if vehicle_no:
                    store_vehicle_location_history(
                        vehicle_no, current_lat, current_lon, int(timestamp))

                # Get route stops information
                stopsInfo = stop_tracker.get_route_stops(route_id)

                # Get visited stops to track progress
                visited_stops = stop_tracker.get_visited_stops(
                    route_id, device_id)
                ...
                logger.error(
                    f"Error processing GPS point for device {device_id}: {e}")
                traceback.print_exc()
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Travel time cache contains {len(cache_data)} segment entries.")
