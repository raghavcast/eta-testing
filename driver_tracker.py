...
class DriverTracker:
    def __init__(self, 
                 ...
                 output_file: str = "driver_movements.csv"):
        ...
        self.driver_movements = []  # List of movement records
        self.output_file = output_file
        
        # Initialize movement DataFrame
        self.movements_df = pd.DataFrame(columns=[
        ...
        if not os.path.exists(output_file):
            self.movements_df.to_csv(output_file, index=False)
    ...
    def _get_route_info(self, device_id: str, timestamp: datetime) -> Optional[Dict]:
        ...
        try:
            ...
            vehicle_info = self.vehicle_mapping[self.vehicle_mapping['Device Id'] == device_id]
            ...
            schedule_info = self.fleet_schedule.loc[self.fleet_schedule['Fleet No'] == vehicle_number]
            ...
            route_info = self.schedule_route_mapping.loc[self.schedule_route_mapping['Schedule Number'] == schedule_number]
            if route_info.empty:
            ...
            if not direction or direction not in ['UP', 'DOWN']:
                ...
                up_route = self.bus_route_source.loc[
                    (self.bus_route_source['Route Number'] == route_number) & 
                    (self.bus_route_source['Route Direction'] == 'UP')
                ]
                down_route = self.bus_route_source.loc[
                    (self.bus_route_source['Route Number'] == route_number) & 
                    (self.bus_route_source['Route Direction'] == 'DOWN')
                ]
            ...
            route_data_info = self.route_finder.df.loc[self.route_finder.df['Route Number'] == route_number]
            ...
            route_details = self.route_finder.get_route_info(route_number, direction, route_id)
    ...
    def get_route_order(self, stop_name: str, route_id: str, direction: str) -> Optional[int]:
        ...
        try:
            ...
            route_number = self.route_finder.get_route_number(route_id)
            if not route_number:
                return None
            
            # Get order from bus_route_source
            order_info = self.bus_route_source[
                (self.bus_route_source['Route Number'] == route_number) &
                (self.bus_route_source['Stop Name'] == stop_name) &
                (self.bus_route_source['Direction'] == direction)
            ]
    ...
    def update_position(self, device_id: str, lat: float, lon: float, timestamp: str,
        ...
        try:
            ...
            try:
                nearest_stop_info = self.route_finder.find_nearest_stop(current_point, route_info)
                if not nearest_stop_info:
                    logger.warning(f"No nearest stop found for device {device_id} at {timestamp}")
                    return
            except Exception as e:
            ...
            # Skip if too far from route (more than 500m)
            if distance > 0.5:
            ...
            if not prev_position or prev_position[0] != current_stop:
                ...
                if prev_position:
                    ...
                    prev_coords = self.route_finder.get_stop_coordinates(prev_stop)
                    curr_coords = self.route_finder.get_stop_coordinates(current_stop)
                    ...
                    if prev_coords and curr_coords:
                        ...
                        route_stops_up = self.bus_route_source[
                            (self.bus_route_source['Route Number'] == route_info['route_number']) &
                            (self.bus_route_source['Route Direction'] == 'UP')
                        ]
                        route_stops_down = self.bus_route_source[
                            (self.bus_route_source['Route Number'] == route_info['route_number']) &
                            (self.bus_route_source['Route Direction'] == 'DOWN')
                        ]
                        ...
                        if len(self.position_buffers[buffer_key]) >= 2:
                            ...
                            first_stop_info = self.route_finder.find_nearest_stop((first_point[0], first_point[1]), route_info)
                            last_stop_info = self.route_finder.find_nearest_stop((last_point[0], last_point[1]), route_info)
                            ...
                            if first_stop_info and last_stop_info:
                                ...
                                if actual_direction and prev_order is not None and curr_order is not None and time_diff >= 1:
                                    ...
                                    # Write movement to CSV
                                    with open(self.output_file, 'a', newline='') as f:
                                        writer = csv.DictWriter(f, fieldnames=self.movements_df.columns)
    ...
    def _write_pending_movements(self):
        """Write pending movements to CSV file."""
        if not self.pending_movements:
            return
            
        # Convert to DataFrame
        new_movements = pd.DataFrame(self.pending_movements)
        ...
        # Write to CSV, overwriting the entire file with the complete DataFrame
        self.movements_df.to_csv(self.output_file, index=False)
